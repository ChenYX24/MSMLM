...
class SaveMolAwareCallback(TrainerCallback):
    """
    每次 on_save / on_train_end，把 Llama 与额外模块分开保存：
    - <ckpt_dir>/llm/ 下是标准 HF 权重（model.safetensors 或 bin）、config 等
    - <ckpt_dir>/extras/ 下是 gvp_encoder.pt / mol_adapter.pt / diffusion_adapter.pt
    - <ckpt_dir>/molaware_metadata.json 描述额外模块文件名，便于 from_pretrained 自动加载
    """
    def _save_one(self, args, state, model, tokenizer, final=False):
        if final or state.global_step is None:
        ...
        # 1) 保存 Llama（只保存底座，不混入自定义模块）
        try:
            model.llm.save_pretrained(llm_dir)
        except Exception as e:
            print(f"[SaveMolAwareCallback] WARN: save LLM failed: {e}")

        try:
            if tokenizer is not None:
                tokenizer.save_pretrained(ckpt_dir)
            if getattr(model, "config", None) is not None:
                model.config.to_json_file(os.path.join(ckpt_dir, "config.json"))
        except Exception as e:
            print(f"[SaveMolAwareCallback] WARN: save tokenizer/config failed: {e}")

        extras = {}
        def _maybe_save(name, module, filename):
            if hasattr(model, name) and getattr(model, name) is not None:
                try:
                    path = os.path.join(extras_dir, filename)
                    torch.save(getattr(model, name).state_dict(), path)
                    extras[name] = os.path.relpath(path, ckpt_dir)  
                except Exception as e:
                    print(f"[SaveMolAwareCallback] WARN: save {name} failed: {e}")

        _maybe_save("gvp_encoder", model.gvp_encoder, "gvp_encoder.pt")
        _maybe_save("mol_adapter", model.mol_adapter, "mol_adapter.pt")
        _maybe_save("diffusion_adapter", model.diffusion_adapter, "diffusion_adapter.pt")

        # 体积很大、且通常只在推理用到的 diffusion 主体，**默认不保存**。
        # 如需保存，解除下面注释（注意体积/速度）：
        # if hasattr(model, "diffusion") and model.diffusion is not None:
        #     try:
        #         path = os.path.join(extras_dir, "diffusion.pt")
        #         torch.save(model.diffusion.state_dict(), path)
        #         extras["diffusion"] = os.path.relpath(path, ckpt_dir)
        #     except Exception as e:
        #         print(f"[SaveMolAwareCallback] WARN: save diffusion failed: {e}")

        # 3) 写 metadata（告诉 from_pretrained 去哪里找额外模块）
        metadata = {
        ...
        with open(os.path.join(ckpt_dir, "molaware_metadata.json"), "w", encoding="utf-8") as f:
            json.dump(metadata, f, ensure_ascii=False, indent=2)

        print(f"[SaveMolAwareCallback] ✅ Saved LLM to {llm_dir}, extras to {extras_dir}")

    def on_save(self, args, state, control, **kwargs):
        self._save_one(args, state, kwargs["model"], kwargs.get("tokenizer"))

    def on_train_end(self, args, state, control, **kwargs):
        self._save_one(args, state, kwargs["model"], kwargs.get("tokenizer"), final=True)
