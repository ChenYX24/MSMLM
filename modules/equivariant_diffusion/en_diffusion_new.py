...
class DistributionNodes:
    def __init__(self, histogram):
        """
        单变量 DistributionNodes，用于仅建模 ligand 的原子数分布。

        参数:
            histogram: 1D array-like，表示每个原子数出现的频率或次数。
                       如 histogram[0] 表示“原子数为 5”这种类别出现的频率。
        """
        histogram = torch.tensor(histogram).float()
        histogram = histogram + 1e-3  # 数值稳定性
        self.prob = histogram / histogram.sum()
        self.m = torch.distributions.Categorical(self.prob)

        self.entropy = self.m.entropy().item()
        print("Entropy of ligand size distribution: H[N]", self.entropy)

    def sample(self, n_samples=1):
        """
        从分布中采样原子数类别索引（索引需映射回实际原子数）
        """
        return self.m.sample((n_samples,))

    def log_prob(self, batch_sizes):
        """
        返回给定原子数索引的 log_prob。
        参数:
            batch_sizes: tensor of shape [B]，每个值为类别索引（如 0 表示原子数为 5）
        """
        return self.m.log_prob(batch_sizes.to(dtype=torch.long)).to(batch_sizes.device)
