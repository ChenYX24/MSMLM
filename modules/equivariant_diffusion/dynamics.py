...
class LigandOnlyEGNNDynamics(nn.Module):
    def __init__(self, atom_nf, n_dims, embedding_dim,
                 joint_nf=16, hidden_nf=64, device='cpu',
                 act_fn=torch.nn.SiLU(), n_layers=4, attention=False,
                 condition_time=True, tanh=False, mode='egnn_dynamics',
                 norm_constant=0, inv_sublayers=2, sin_embedding=False,
                 normalization_factor=100, aggregation_method='sum',
                 edge_cutoff=None, reflection_equivariant=True):
        super().__init__()
        ...
        dynamics_node_nf = joint_nf + embedding_dim + (1 if condition_time else 0)
        self.egnn = EGNN(
    ...
    def forward(self, xh_atoms, t, mask_atoms, condition=None):
        x_atoms = xh_atoms[:, :self.n_dims].clone()
        h_atoms = xh_atoms[:, self.n_dims:].clone()
        ...
        if condition is not None:
            if condition.dim() == 2:
                cond = condition[mask_atoms]  # [N, D]
            elif condition.dim() == 3:
                cond = condition.squeeze(1)[mask_atoms]  # [N, D]
            else:
                raise ValueError(f"Unexpected condition shape: {condition.shape}")

            h_atoms = torch.cat([h_atoms, cond], dim=1)

        if self.condition_time:
            if t.numel() == 1:
                h_time = torch.full((h_atoms.size(0), 1), float(t.item()), device=h_atoms.device)
            else:
                h_time = t[mask_atoms].float().view(-1, 1)
            h_atoms = torch.cat([h_atoms, h_time], dim=1)

        edges = self.get_edges(mask_atoms, x_atoms)

        h_final, x_final = self.egnn(h_atoms, x_atoms, edges, batch_mask=mask_atoms)
        vel = x_final - x_atoms

        decode_input_dim = h_final.size(1) - (1 if self.condition_time else 0) - self.embedding_dim
        h_final_atoms = self.atom_decoder(h_final[:, :decode_input_dim])
